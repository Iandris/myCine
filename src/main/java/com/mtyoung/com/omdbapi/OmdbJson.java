...
public class OmdbJson {
    public List<Movie> searchByTitle(String movieTitle) throws Exception  {
        ...
        WebTarget target =
                client.target("http://www.omdbapi.com/?r=json&type=movie&s=" + movieTitle + "&page=" + 1);
        ...
        int totalresults = Integer.parseInt(search.getTotalResults());

        if (totalresults > 10) {
            for(int i = 2; i < ((totalresults / 10) + 1); i++) {
                WebTarget tar =
                        client.target("http://www.omdbapi.com/?r=json&type=movie&s=" + movieTitle + "&page=" + i);
                String resp = tar.request(MediaType.APPLICATION_JSON).get(String.class);

                searches.add(mapper.readValue(resp, Search.class));
            }
        }
